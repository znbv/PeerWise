...
urlpatterns = [
    path(
        "", views.TutorListView.as_view(), name="tutors"
    ),  # tutors is the name of the URL pattern, which can be used in templates to link to this view
    path(
        "request/<int:pk>/",
        views.StudentRequestCreateView.as_view(),
        name="student_request",
    ),
    path(
        "feedback/<int:pk>/", views.FeedbackCreateView.as_view(), name="leave_feedback"
    ),
